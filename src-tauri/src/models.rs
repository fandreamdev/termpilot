...
#[derive(Debug, Serialize)]
pub struct Envelope<T: Serialize> { pub ok: bool, pub request_id: String, pub data: Option<T>, pub error: Option<ErrorBody> }
impl<T: Serialize> Envelope<T> { pub fn ok(data: T) -> Self { Self { ok: true, request_id: uuid::Uuid::new_v4().to_string(), data: Some(data), error: None } } pub fn err(code: &str, message: &str) -> Self { Self { ok: false, request_id: uuid::Uuid::new_v4().to_string(), data: None, error: Some(ErrorBody { code: code.into(), message: message.into() }) } } }
